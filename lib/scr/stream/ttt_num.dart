...
extension RxNumExt<T extends num> on TttStream<T> {
...
extension TttNumExt<T extends num> on TttStream<T?> {
...
class TttNum extends TttStream<num> {
  TttNum(super.initial);
...
class TttNumNullable extends TttStream<num?> {
  TttNumNullable([super.initial]);
...
class TttNullableInt extends TttStream<int?> {
  TttNullableInt([super.initial]);

  /// Addition operator.
  TttNullableInt operator +(int other) {
    if (value != null) {
      value = value! + other;
    }
    return this;
...
extension TttNullableIntExt on TttStream<int?> {
  /// Bit-wise and operator.
  ///
  /// Treating both `this` and [other] as sufficiently large two's component
  /// integers, the result is a number with only the bits set that are set in
